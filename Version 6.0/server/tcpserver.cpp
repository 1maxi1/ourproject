...
bool tcpServer::parse(QTcpSocket *pSocket)
{
...

    {
        ...
        if(listLoginPassword.length() == 2)
        {
            ...
            query.prepare("SELECT id, role_id FROM users WHERE login = :login AND password=:pwd;");
            ...
            while (query.next())
            {
                ...
                currentUserRole = query.value(1).toInt();
    ...
    if(cmd == "login")
    {
        ...
        while (query.next())
        {
            ...
            if(query.value(2).toString() == password)
            {
                ...
                currentUserId = query.value(0).toInt();
                currentUserRole = query.value(4).toInt();
            }
        }

        qDebug() << "LOGIN RESULT:" << isLogined;
        sendMessage(pSocket, "auth", (isLogined ? "ok:" : "err:") + QString::number(currentUserRole));
    }
    else if(cmd == "register")
    {
        ...
        query.clear();
        query.prepare("INSERT INTO users (login, password, email, role) VALUES (:login, :password, :email, 1);");
        query.bindValue(":login", login);
    ...
    else if(cmd == "get_stat")
    {
        ...
        {
            sendMessage(pSocket, "err", "Недостаточно прав");
            return true;
        }

        QSqlQuery query(singleton::getInstance()->database());
        query.prepare("select user_id, users.login AS login, sum(ok) AS ball from solution, users where user_id=users.id group by user_id;");
        query.exec();
        ...
        while (query.next())
        {
            msgToSend += QString(query.value(1).toString().toUtf8().toBase64()) + ":"+ query.value(2).toString() + ";";
        }

        sendMessage(pSocket, "stat", msgToSend);
    }
