...
singleton::singleton()
{
    ...
    query.prepare("CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY AUTOINCREMENT, login TEXT UNIQUE, password TEXT, email TEXT, role_id INTEGER);");
    query.exec();
    ...
    query.clear();
    query.prepare("CREATE TABLE IF NOT EXISTS solution(user_id INTEGER, task_name TEXT, input_params TEXT, ans TEXT, ok INTEGER);");
    query.exec();

    query.clear();
    query.prepare("CREATE TABLE IF NOT EXISTS roles(id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT);");
    query.exec();
